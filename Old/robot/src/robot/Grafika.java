...
public class Grafika extends Applet implements KeyListener, Runnable // dziediczy
																		// klase
// Applet i
// interfejs
// KeyListener
{
	...
	private void wczytajObrazki() { // paleta barw - st¹d pobiera kolory do
		...
		for (int i = 0; i < 15; i++) // bo mamy 14 obrazkow
		{
	...
	public void paint(Graphics g) // rysowanie planszy kwadratow
	{
		try {
			...
			for (int y = 0; y < m.tablica_wyswietlana.length; y++) { // rysowanie
																		// po
																		// y
																		// (kolejne
																		// wiersze)
				for (int x = 0; x < m.tablica_wyswietlana[0].length; x++) { // rysowanie
					...
					if (m.tablica_wyswietlana[x][y] < 0) {
						...
						if (m.tablica_wyswietlana[x][y] < 128) {
							...
							g.setColor(new Color(255 - ((m.tablica_wyswietlana[x][y] - 128) * 2),
									255 - ((m.tablica_wyswietlana[x][y] - 128) * 2),
									255 - ((m.tablica_wyswietlana[x][y] - 128) * 2)));
							g.fillRect(50 + x * w1, 50 + y * w2, w1, w2);
						}
					}
					...
					// kolumnach)
					// g.drawImage(obrazki[m.tablica_wyswietlana[x][y]], 50 + x
					// *
					// w1, 50 + y * w2, w1, w2, applet); // rysowanie
					// kolorze
					// g.drawImage(img, x, y, width, height, observer)
			}
			if (this.wczytanoMapeArchiwalna == 1) {
				ma.rysujplan();
				for (int y = 0; y < ma.tablica_wyswietlana.length; y++) { // rysowanie
																			// po
					// y
					// (kolejne
					// wiersze)
					for (int x = 0; x < ma.tablica_wyswietlana[0].length; x++) { // rysowanie
						...
						if (ma.tablica_wyswietlana[x][y] < 0) {
							...
							if (ma.tablica_wyswietlana[x][y] < 128) {
								...
								g.setColor(new Color(255 - ((ma.tablica_wyswietlana[x][y] - 128) * 2),
										255 - ((ma.tablica_wyswietlana[x][y] - 128) * 2),
										255 - ((ma.tablica_wyswietlana[x][y] - 128) * 2)));
								g.fillRect(500 + x * w1, 50 + y * w2, w1, w2);
							}
						}
						...
						// kolumnach)
						// g.drawImage(obrazki[m.tablica_wyswietlana[x][y]], 50
						// + x
						// * w1, 50 + y * w2, w1, w2, applet); // rysowanie
						// kolorze
						// g.drawImage(img, x, y, width, height, observer)
	...
	public void keyReleased(KeyEvent e) {
		...
		case 'w':
			...
			{
				if (wczytanoMapeArchiwalna == 1)
					ma.ruchBezSystemu(1, false, false, 0);
		...
		case 's':
			...
			if (m.ruchzSystemem(0, false, false, 1)) {
				if (wczytanoMapeArchiwalna == 1)
					ma.ruchBezSystemu(0, false, false, 1);
		...
		case 'a':
			...
			if (m.ruchzSystemem(0, true, false, 0)) {
				if (wczytanoMapeArchiwalna == 1)
					ma.ruchBezSystemu(0, true, false, 0);
		...
		case 'd':
			...
			if (m.ruchzSystemem(0, false, true, 0)) {
				if (wczytanoMapeArchiwalna == 1)
					ma.ruchBezSystemu(0, false, true, 0);
				// command = Integer.toString(-21) + " " + "0";
				// try {
				// sterowanie.sendPacket(command);
				// }
	...
	public void run() {
		...
		String command = "0 0";
		if (zerowanie_sterowania == 100) { // opoznnienie nadawania=czas na
											// wykonuwanie ruchu - DO USTAWIENIA
			try {
				sterowanie.sendPacket(command);
			} catch (IOException e1) {
				// TODO Auto-generated catch block
