...
public class Mapa extends TimerTask {
	...
	private int[][] tablica_robocza; // tablica glowna (robocza), przechowuje
										// dane o wszytkich polach
	public int[][] tablica_wyswietlana; // z tego program rysuje nasza tablice
										// 20*20 wypelniona bloczkami
	private int haczykx, haczyky;// punkty odniesienia polozone centralnie na
									// mapie 20*20
	...
	public void odswierzCzujniki(int lewo, int front, int prawo, int tyl) {
		...
		if (tablica_robocza[robot.X() + tab[0]][robot.Y() + tab[1]] != -1) {
			if (tablica_robocza[robot.X() + tab[0]][robot.Y() + tab[1]] < 255) {
				tablica_robocza[robot.X() + tab[0]][robot.Y()
						+ tab[1]] = tablica_robocza[robot.X() + tab[0]][robot.Y() + tab[1]] + 1;
			}
		}
		tab = new int[2];
	...
	public void rysujplan() // z tablicy roboczej przekazuje dane do rysowanej
							// tablicy 20*20
	{
		...
		for (int i = 0; i < tablica_wyswietlana.length; i++) {
		 ...
		 if(abs(robot.X()-haczykx)>7||abs(robot.Y()-haczyky)>7) 
			 // sprawdzenie czy robot nie jest poza dozwolonym obszarem (pole 7*7)
		 {
		 haczykx=robot.X();
		 haczyky=robot.Y();
		 }
		 ...
		 for(int i=0;i<tablica_wyswietlana.length;i++){
		 for(int j=0;j<tablica_wyswietlana.length;j++){
		 tablica_wyswietlana[i][j] =
		 tablica_robocza[i+haczykx-10][j+haczyky-10];
		 }
		 }
	}
	
	public void zapisz(String nazwa) throws FileNotFoundException 
	{
	...
	public void wczytaj(String nazwa) throws FileNotFoundException {
		...
		for (int i = 0; i < wielkosc; i++) {
			for (int j = 0; j < wielkosc; j++) {
				tablica_robocza[j][i] = odczyt.nextInt();
				if (tablica_robocza[j][i] < -1) {
					tablica_robocza[j][i] = 0;
				}
			}
		}
		tablica_robocza[robot.X()][robot.Y()] = robot.V();
...
//		 if(z==20) // { if(kier==1){ 
//		  ruch(1); // odswierzCzujniki(50,50,50,50); u++; if(u>15) {ruch(2);
//		  u=0; }
//		  }
//		  z=0; }
//		// generuj();
	...
	/*
	 * kolory obraz do testowania grafiki aplikacji private void generuj() {
	 * for(int i=0;i<wielkosc;i++){ for(int j=0;j<wielkosc;j++){ plan[i][j]=
	 * gen.nextInt(10); } } }
	 */
